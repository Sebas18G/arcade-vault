...
**Dos proyectos de Supabase.** Desarrollo (`okqmxxqnmcqpqzusnype`) es el que expone el MCP de `.mcp.json` y el único donde se usa `apply_migration`. Producción es un proyecto aparte que **no** está conectado al MCP: se opera desde el dashboard con los scripts de `supabase/prod/`, siguiendo `references/migracion-produccion.md`.

**Todo cambio de esquema va en una migración.** `supabase/migrations/<version>_<nombre>.sql` es la fuente de verdad del esquema y contiene el historial completo desde el origen (15 migraciones al 2026-09-05). El flujo es: escribir el archivo primero, aplicarlo en dev con `apply_migration` usando ese mismo nombre y SQL, y commitearlo junto al código que lo necesita. **Nunca `execute_sql` para DDL**: no queda en el historial y el archivo y la base se separan en silencio. Producción no se sincroniza sola; las migraciones pendientes se aplican allá a mano y se registran en su `supabase_migrations.schema_migrations`. El detalle completo del flujo, las reglas del SQL (grants explícitos, RLS, `search_path`, revocar `EXECUTE` a `public` y no solo a `anon`/`authenticated`) y la relación con `supabase/prod/bootstrap.sql` están en `supabase/README.md`; leerlo antes de tocar la base.
