...
`app/` implementa Arcade Vault siguiendo las specs aprobadas hasta ahora (`specs/01` a `specs/06`, todas en estado Implementado), traducido desde el mockup de referencia a Next.js/React 19 idiomático:
...
- `components/games/` — motores reales portados desde `references/started_games/` (spec 05), cada uno como componente de canvas autocontenido que notifica puntaje/vidas/nivel/fin de partida a `GamePlayer` vía props/callbacks (`components/games/shared/types.ts`):
  ...
  - `tetris/` — `TetrisCanvas` + `engine.ts` + `NextPieceCanvas` + `tetris.module.css` (tema claro/oscuro y skin escopeados al contenedor del juego, nunca a `:root`/`document.body`) + `leaderboard.ts` (top-5 vía Supabase, tabla `tetris_scores`, spec 06; `tetris-best-stats`, `tetris-theme`, `tetris-skin`, `tetris-start-level` siguen en `localStorage`).
