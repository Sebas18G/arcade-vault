...
- `lib/supabase/client.ts` / `lib/supabase/server.ts` — clientes de Supabase (navegador/servidor) conectados al proyecto ya referenciado en `.mcp.json` (spec 04), configurados con `db: { schema: "arcade-vault" }` (spec 06): este proyecto **no expone el schema `public` por la Data API**, solo `graphql_public` y `arcade-vault` — cualquier tabla nueva debe crearse en `"arcade-vault"` (con el schema entre comillas dobles en SQL, por el guion) o quedará invisible para `supabase-js`/PostgREST aunque exista y sea consultable por SQL directo. Todavía sin auth real ni edge functions.
...
- `components/games/` — motores reales portados desde `references/started_games/` (spec 05), cada uno como componente de canvas autocontenido que notifica puntaje/vidas/nivel/fin de partida a `GamePlayer` vía props/callbacks (`components/games/shared/types.ts`):
  - `asteroids/` — `AsteroidsCanvas` + `engine.ts` + `leaderboard.ts` (top-5 vía Supabase, tabla `asteroids_scores`, spec 06; `asteroids_player_name` sigue en `localStorage` como preferencia de UI).
  - `tetris/` — `TetrisCanvas` + `engine.ts` + `NextPieceCanvas` + `tetris.module.css` (tema claro/oscuro y skin escopeados al contenedor del juego, nunca a `:root`/`document.body`) + `leaderboard.ts` (top-5 vía Supabase, tabla `tetris_scores`, spec 06; `tetris-best-stats`, `tetris-theme`, `tetris-skin`, `tetris-start-level` siguen en `localStorage`). Los controles de SKIN/TEMA viven en el HUD de `GamePlayer`, no en `TetrisCanvas` (spec 07): `TetrisCanvas` recibe `skin`/`theme` como props controladas y ya no tiene el `<select>`/switch que antes flotaba sobre el canvas.
  - `arkanoid/` — `ArkanoidCanvas` + `engine.ts` (assets en `public/games/arkanoid/`) + `leaderboard.ts` (top-5 vía Supabase, tabla `arkanoid_scores`, spec 06); el modal de fin de juego ya no es `scoreOnly` — tiene campo de nombre y top-5 igual que los otros dos.
