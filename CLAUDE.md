...
- `components/games/shared/skins.ts` — contrato de skins compartido: el tipo `GameSkin` (`classic` / `retro` / `neon`), `GAME_SKINS` (valores en inglés, etiquetas de UI en Español) y el validador `isGameSkin()`. `GameCanvasProps` (en `shared/types.ts`) declara `skin?: GameSkin` como **opcional**, porque `arkanoid` y `snake` todavía no están migrados. Un juego "tiene skins" cuando cumple las tres cosas: `skins.ts` con las paletas, `setSkin()` en su `engine.ts` y prop `skin` en su `<id>-canvas.tsx` propagada al motor. Al día de hoy lo cumplen `asteroids` (las 3 compartidas) y `tetris` (4 propias: `retro`, `neon`, `pastel`, `pixel`, anteriores al contrato y con su propio tipo `TetrisSkin`). Reglas: la skin `classic` debe reproducir exactamente el aspecto original del juego; los motores reciben la paleta por `setSkin()` y **nunca** leen `document`/`window`/`localStorage`; las preferencias de skin van a `localStorage` desde el `leaderboard.ts` del juego, nunca a Supabase.
...
`references/skin-status.md` es la **bitácora de skins**: la memoria persistente del agente `skin-designer` (tabla índice + una ficha por juego, con estados `Cumple` / `Pendiente` / `Exento`), con las paletas ya decididas y sus razones de contraste. Estado actual: `asteroids` **Cumple**, `tetris` **Exento**, `arkanoid` y `snake` **Pendientes**. También es editable a mano; si contradice al código, gana el código.
