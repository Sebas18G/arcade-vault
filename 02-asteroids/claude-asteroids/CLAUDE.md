...
- **Power-ups**: clase `PowerUp` (game.js:297-341), tipos `triple`/`shield`/`slowmo`/`hyper` (definidos en `POWERUP_STYLES`, cada uno con su `shape` de dibujo — diamante o hexágono) con 20% de probabilidad al destruir un asteroide (`POWERUP_CHANCE`), garantizados como máximo una vez por nivel (`powerupSpawnedThisLevel`); si el nivel se limpia sin que haya salido ninguno, se fuerza en la posición del último asteroide destruido (game.js:461-464). `hyper` (hiperpropulsión) multiplica el `THRUST` y reduce el `DRAG` de la nave durante `ship.hyperTimer` (~8s), aumentando drásticamente aceleración y velocidad máxima.
