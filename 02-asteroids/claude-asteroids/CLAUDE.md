...
- **Power-ups**: clase `PowerUp`, tipos `triple`/`shield`/`slowmo`/`nova` con 20% de probabilidad al destruir un asteroide (`POWERUP_CHANCE`), garantizados como máximo una vez por nivel (`powerupSpawnedThisLevel`); si el nivel se limpia sin que haya salido ninguno, se fuerza en la posición del último asteroide destruido. El tipo se sortea de forma uniforme entre los cuatro (`randomPowerUpType`); la escasez de `nova` como "ítem escaso" viene solo del límite de un power-up garantizado por nivel, no de un peso menor. `nova` (Bomba Nova) es de un solo uso: al recogerla, `triggerNovaBomb()` destruye instantáneamente todos los asteroides activos (sin fragmentarlos vía `split()`) y dispara una onda expansiva visual (`novaFlash`/`novaOrigin`, dibujada en `draw()`); los demás tipos activan temporizadores en la nave (`ship.tripleShotTimer`/`shieldTimer`/`slowMoTimer`). Visualmente, cada tipo puede tener una forma distinta (`style.shape`: `diamond` o `hexagon`) y parpadear (`style.blink`) — `nova` usa hexágono parpadeante para diferenciarse del resto sin romper la estética wireframe.
