...
- El overlay `#overlay` (en `index.html`, junto al canvas dentro de `.board-wrap`) tiene dos botones: `#continue-btn` (llama a `togglePause()` si `paused` es `true`) y `#restart-btn` (pone `paused = false`, oculta el overlay y llama a `startGame()`).
...
- El listener global de `keydown`/`keyup` ignora las pulsaciones cuando `document.activeElement` es un `<input>` (para no interferir con el campo de nombre ni disparar disparo/pausa mientras se escribe); `#player-name-input` tiene su propio listener de `Enter` para guardar sin pasar por el input global de juego.
