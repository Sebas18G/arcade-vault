...
- `app/salon/page.tsx` reescrito (misma ruta `/salon`, no se renombra):
  ...
  - Tab adicional `GLOBAL` (siempre presente, no depende de `games`): top-20 de `global_scores`, con un filtro por juego (chips: TODOS/ASTEROIDS/TETRIS/ARKANOID) que aplica `eq('game_id', ...)` sobre la misma tabla.
  - "TU MEJOR MARCA" (fila destacada del usuario actual) se calcula con datos reales: si hay sesión (`useAuth`), query adicional `eq('player_name', user.name)` + `order('score', {ascending:false})` + `limit(1)` sobre la tabla del tab activo (o `global_scores` en el tab GLOBAL); si no hay fila, no se muestra la sección.
  - Suscripción Realtime (`postgres_changes`, evento `INSERT`) sobre la tabla del tab activo, filtrada por `game_id` cuando aplica (tab GLOBAL); nuevas filas se insertan/reordenan en vivo en el top visible sin recargar la página. Se desuscribe al cambiar de tab o desmontar.
...
7. Reescribir `app/salon/page.tsx`: fetch de `games` para los tabs, fetch client-side de top-12 al cambiar de tab, tab `GLOBAL` con top-20 + filtro por juego, "tu mejor marca" con datos reales, suscripción Realtime por tab activo con cleanup al cambiar/desmontar. Verificación manual: `/salon` muestra solo 4 tabs (Asteroids/Tetris/Arkanoid/Global); jugar una partida en otra pestaña del navegador y confirmar que el puntaje nuevo aparece en el Salón sin recargar la página.
...
- [ ] `/salon` muestra exactamente 4 tabs: uno por cada juego presente en la tabla `games` (los 3 reales) más el tab `GLOBAL`. Los 5 juegos simulados no tienen tab.
- [ ] El tab `GLOBAL` muestra hasta 20 filas cruzando los 3 juegos, y el filtro por juego (chips) reduce la lista a `game_id` seleccionado.
- [ ] Jugar una partida en una pestaña del navegador y guardarla hace aparecer el nuevo puntaje en `/salon` (tab del juego y tab `GLOBAL`) abierto en otra pestaña, sin recargar la página (verificado con la suscripción Realtime).
- [ ] Con sesión iniciada, "TU MEJOR MARCA" en cada tab de juego real y en el tab `GLOBAL` refleja el mejor puntaje real guardado con ese nombre (no un valor inventado); si no hay ninguno, la sección no se muestra.
...
- **Sí (decisión del autor de la spec, ajustable):** conteos de top-N: 5 en el modal de fin de juego (igual que el límite que ya usaban Asteroids/Tetris en `localStorage`), 12 en cada tab de juego del Salón (confirmado explícitamente por el usuario) y en la ficha de detalle, 20 en el tab `GLOBAL`. No se volvió a preguntar por ser un detalle de despliegue visual de bajo riesgo; se puede ajustar editando la spec antes de aprobarla.
- **Sí (decisión del autor de la spec, ajustable):** "TU MEJOR MARCA" se implementa con datos reales (`player_name` igual al nombre de sesión) tanto en los 3 tabs de juego real como en el tab `GLOBAL` del Salón. Es la continuación natural de reemplazar `seededScores`; se puede revertir si el usuario prefiere ocultar esa sección para los juegos reales.
