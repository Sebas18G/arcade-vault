...
- **Pausa** (`togglePause`): alterna la variable `paused` (independiente de `gameOver`; no hace nada si `gameOver` es `true`). Al pausar, cancela el `requestAnimationFrame` (`cancelAnimationFrame(animId)`) y muestra el overlay `#overlay` con el título "PAUSA" y el botón `#continue-btn` visible (`continueBtn.hidden = false`). Al reanudar, hay que **ocultar explícitamente el overlay** (`overlay.classList.add('hidden')`) además de relanzar el loop (`loop(lastTime)`) — omitir esto deja el overlay visible tapando el tablero aunque el juego siga corriendo por debajo (bug ya corregido una vez, cuidado si se toca esta función). El overlay se comparte con la pantalla de "GAME OVER" (`endGame()`), que oculta `#continue-btn` (`continueBtn.hidden = true`) porque desde ahí no se puede continuar, solo reiniciar (`#restart-btn` → `init()`).
