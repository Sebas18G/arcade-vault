...
**Situación:** `components/games/arkanoid/` tiene solo `engine.ts`, `arkanoid-canvas.tsx` y `leaderboard.ts`. Sin `skins.ts`, sin `setSkin()`, sin prop `skin`. No figura en el registro `SKINS_BY_GAME` de `components/game-player.tsx`, así que su HUD todavía no muestra el botón "SKIN" (correcto: no tendría efecto). Reconfirmado en la corrida 3 (2026-09-04): sigue igual y vuelve a ser **el único pendiente del alcance**, así que es el objetivo por defecto de la próxima corrida.
