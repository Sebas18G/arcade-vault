...
alter table asteroids_scores enable row level security;
alter table tetris_scores enable row level security;
alter table arkanoid_scores enable row level security;
...
create policy "asteroids_scores_select_public" on asteroids_scores
  for select to anon, authenticated using (true);
create policy "tetris_scores_select_public" on tetris_scores
  for select to anon, authenticated using (true);
create policy "arkanoid_scores_select_public" on arkanoid_scores
  for select to anon, authenticated using (true);
create policy "global_scores_select_public" on global_scores
  for select to anon, authenticated using (true);

-- INSERT público solo en las 3 tablas de juego (games y global_scores sin policy de INSERT para el cliente)
create policy "asteroids_scores_insert_public" on asteroids_scores
  for insert to anon, authenticated
  with check (char_length(player_name) between 1 and 10 and score >= 0);
create policy "tetris_scores_insert_public" on tetris_scores
  for insert to anon, authenticated
  with check (char_length(player_name) between 1 and 10 and score >= 0);
create policy "arkanoid_scores_insert_public" on arkanoid_scores
  for insert to anon, authenticated
  with check (char_length(player_name) between 1 and 10 and score >= 0);
...
alter publication supabase_realtime add table asteroids_scores;
alter publication supabase_realtime add table tetris_scores;
alter publication supabase_realtime add table arkanoid_scores;
